...
<template>
  <div class="space-y-6">
    <header>
      <h1 class="text-2xl font-serif text-blue-600">
        Valibot Traits Demo
      </h1>
      <p class="text-gray-500">
        Demonstrating same example traits using Valibot schemas.
      </p>
    </header>

    <div
      v-if="article"
      class="border p-6 rounded-lg shadow-sm bg-white border-blue-100"
    >
      <div
        v-if="hasTrait('seo')"
        class="mb-4"
    ...
    <div class="mt-8 pt-8 border-t text-xs font-mono text-gray-400">
      <h3 class="uppercase font-bold mb-2 text-[10px]">
        Debug Meta (Inferred via wrapper)
      </h3>
